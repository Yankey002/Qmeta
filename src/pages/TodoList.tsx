...
const TodoList: React.FC = () => {
  ...
  const getPriorityInfo = (priority: string) => {
    switch(priority) {
      case 'high':
        return { color: 'bg-red-500', bgColor: 'bg-red-50 dark:bg-red-900/20', textColor: 'text-red-600 dark:text-red-400' };
      case 'medium':
        return { color: 'bg-amber-500', bgColor: 'bg-amber-50 dark:bg-amber-900/20', textColor: 'text-amber-600 dark:text-amber-400' };
      case 'low':
        return { color: 'bg-green-500', bgColor: 'bg-green-50 dark:bg-green-900/20', textColor: 'text-green-600 dark:text-green-400' };
      default:
        return { color: 'bg-gray-500', bgColor: 'bg-gray-50 dark:bg-gray-800', textColor: 'text-gray-600 dark:text-gray-400' };
    }
  };
  ...
  return (
    <div className="p-6"><div className="flex flex-col sm:flex-row sm:items-center justify-between mb-6 gap-4"><h1 className="text-2xl font-bold text-gray-800 dark:text-white">待办事项</h1>
        ...
        <div className="flex flex-wrap gap-2">
          ...
          <div className="flex border border-gray-300 dark:border-gray-600 rounded-lg overflow-hidden">
            ...
            <button 
              className={`px-3 py-2 ${viewMode === 'kanban' 
                ? 'bg-white dark:bg-gray-800 shadow-sm' 
                : 'bg-gray-50 dark:bg-gray-750 hover:bg-gray-100 dark:hover:bg-gray-700'
              } transition-colors`}
                      ...
                      <div className="flex items-start gap-3">
                        ...
                        >
                          {todo.status === 'completed' ? (
                            ...
                            <Square className="text-gray-400 hover:text-gray-600 dark:hover:text-gray-200 transition-colors" size={20} />
                          )}
                        </button>
                        
                        <div className="flex-grow">
                          <div className="flex flex-wrap items-center gap-2 mb-2">
                            <h3 className={`font-medium ${todo.status === 'completed' ? 'line-through text-gray-500 dark:text-gray-400' : 'text-gray-800 dark:text-white'}`}>
                              {todo.title}
                            </h3>
                            <span className={`inline-flex items-center text-xs px-2 py-0.5 rounded-full ${priorityInfo.bgColor} ${priorityInfo.textColor}`}>
                          ...
                          {todo.description && (
                            <p className={`text-sm mb-3 ${todo.status === 'completed' ? 'text-gray-500 dark:text-gray-400' : 'text-gray-600 dark:text-gray-300'}`}>
                              {todo.description}
                            </p>
                          )}
                  ...
                  return (
                    ...
                    >
                      <div className="flex items-center justify-between mb-2">
                        <h3 className="font-medium text-gray-800 dark:text-white text-sm">{todo.title}</h3>
                        <span className={`inline-flex items-center text-xs px-2 py-0.5 rounded-full ${priorityInfo.bgColor} ${priorityInfo.textColor}`}>
              ...
              {groupedTodos.completed.length > 0 ? (
                groupedTodos.completed.map((todo) => {
                  ...
                  return (
                    ...
                    >
                      <div className="flex items-center justify-between mb-2">
                        <h3 className="font-medium text-gray-500 dark:text-gray-400 text-sm line-through">{todo.title}</h3>
                        <span className={`inline-flex items-center text-xs px-2 py-0.5 rounded-full ${priorityInfo.bgColor} ${priorityInfo.textColor} opacity-70`}>
                          <Flag size={10} className="mr-1" />
                          {todo.priority === 'high' ? '高' : todo.priority === 'medium' ? '中' : '低'}
                        </span>
                      </div>
