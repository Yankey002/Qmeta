          ...
          {['即将到来', '全部', '已完成', '已过期'].map((filter) => (
            <button
              ...
              className={`px-3 py-1 text-sm rounded-full ${
                selectedFilter === (filter === '即将到来' ? 'upcoming' : filter === '全部' ? 'all' : filter === '已完成' ? 'completed' : 'past')
                  ? 'bg-blue-100 dark:bg-blue-900/30 text-blue-600 dark:text-blue-400'
                  : 'bg-gray-100 dark:bg-gray-700 text-gray-700 dark:text-gray-300 hover:bg-gray-200 dark:hover:bg-gray-600'
              } transition-colors`}
              onClick={() => setSelectedFilter(filter === '即将到来' ? 'upcoming' : filter === '全部' ? 'all' : filter === '已完成' ? 'completed' : 'past')}
            >
              {filter}
            </button>
          ))}
            ...
            return (
              <motion.div 
                ...
                className={`bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-xl overflow-hidden shadow-sm hover:shadow-md transition-shadow ${
                  !isUpcoming && !reminder.isCompleted ? 'border-red-200 dark:border-red-900/30' : ''
                }`}
                  ...
                  <div className="flex items-start gap-3">
                    {reminder.isCompleted ? (
                      ...
                      >
                        <div className={`w-5 h-5 rounded-full flex items-center justify-center ${
                          !isUpcoming ? 'bg-red-100 dark:bg-red-900/30 border border-red-200 dark:border-red-800' : 'border border-gray-300 dark:border-gray-600'
                        }`}>
                          {!isUpcoming && (
                    ...
                    <div className="flex-grow">
                      <div className="flex flex-wrap items-center gap-2 mb-2">
                        <h3 className={`font-medium ${reminder.isCompleted ? 'line-through text-gray-500 dark:text-gray-400' : 'text-gray-800 dark:text-white'}`}>
                          {reminder.title}
                        </h3>
                      ...
                      {reminder.description && (
                        <p className={`text-sm mb-3 ${reminder.isCompleted ? 'text-gray-500 dark:text-gray-400' : 'text-gray-600 dark:text-gray-300'}`}>
                          {reminder.description}
                        </p>
                      )}
