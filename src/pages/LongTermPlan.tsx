          ...
          {['全部', '高优先级', '中优先级', '低优先级'].map((filter) => (
            <button
              ...
              className={`px-3 py-1 text-sm rounded-full ${
                selectedFilter === (filter === '高优先级' ? 'high' : filter === '中优先级' ? 'medium' : filter === '低优先级' ? 'low' : 'all')
                  ? 'bg-blue-100 dark:bg-blue-900/30 text-blue-600 dark:text-blue-400'
                  : 'bg-gray-100 dark:bg-gray-700 text-gray-700 dark:text-gray-300 hover:bg-gray-200 dark:hover:bg-gray-600'
              } transition-colors`}
              onClick={() => setSelectedFilter(filter === '高优先级' ? 'high' : filter === '中优先级' ? 'medium' : filter === '低优先级' ? 'low' : 'all')}
            >
              {filter}
            </button>
          ))}
                  ...
                  <div className="flex items-start justify-between">
                    <div className="flex-1">
                      <div className="flex flex-wrap items-center gap-2 mb-2">
                        <h3 className="font-semibold text-gray-800 dark:text-white">{plan.title}</h3>
                      ...
                      <div className="flex flex-wrap items-center justify-between gap-4">
                        ...
                        <div className="flex items-center">
                          <span className="text-sm text-gray-500 dark:text-gray-400 mr-2">进度:</span>
                          ...
                          <span className="ml-2 text-sm font-medium text-gray-700 dark:text-gray-300">{plan.progress}%</span>
                        </div>
                      </div>
                    </div>
                        ...
                        {plan.milestones.map((milestone) => (
                          ...
                          >
                            ...
                            <div className="flex-grow">
                              <p className={`text-sm ${milestone.completed ? 'line-through text-gray-500 dark:text-gray-400' : 'text-gray-800 dark:text-gray-200'}`}>
                                {milestone.name}
                              </p>
                            </div>
